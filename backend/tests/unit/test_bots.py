...
class TestCreateBot:
    def test_create_bot_returns_201_and_bot(
        self, client, auth_headers, sample_bot_data
    ):
    ...
    def test_create_bot_without_auth_returns_401(
        self, client, invalid_auth_headers, sample_bot_data
    ):
        response = client.post(
            f"{API_PREFIX}/bots", json=sample_bot_data, headers=invalid_auth_headers
        )
...
class TestGetAllBots:
    def test_get_all_bots_returns_200_and_bot_list(self, client, auth_headers):
        ...
        response = client.get(f"{API_PREFIX}/bots", headers=auth_headers)

        assert response.status_code == status.HTTP_200_OK
        assert isinstance(response.json(), list)

    def test_get_all_bots_without_auth_returns_401(
        self, client, invalid_auth_headers, sample_bot_data
    ):
        response = client.get(f"{API_PREFIX}/bots", headers=invalid_auth_headers)
...
class TestGetBotById:
    ...
    def test_get_bot_by_id_for_nonexistent_bot_returns_404(self, client, auth_headers):
        response = client.get(
            f"{API_PREFIX}/bots/{NONEXISTENT_BOT_ID}", headers=auth_headers
        )

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_get_bot_by_id_without_auth_returns_401(
...
class TestUpdateBotByID:
    def test_update_bot_by_id_single_val_returns_200_and_updated_val(
    ...
    def test_update_bot_by_id_all_vals_returns_200_and_updated_vals(
        self, client, auth_headers, created_bot
    ):
        update_data = {
    ...
    def test_update_bot_by_id_no_data_returns_400(
        self, client, auth_headers, created_bot
    ):
        update_data = {}
    ...
    def test_update_bot_by_id_for_nonexistent_bot_returns_404(
    ...
    def test_update_bot_by_id_without_auth_returns_401(
        self, client, invalid_auth_headers, created_bot
    ):
        update_data = {"bot_name": "Updated Bot"}
...
class TestUpdateBotAPIKey:
    def test_update_bot_api_key_returns_200_and_key(
        self, client, auth_headers, created_bot
    ):
        response = client.patch(
    ...
    def test_update_bot_api_key_for_nonexistent_bot_returns_404(
        self, client, auth_headers
    ):
        response = client.patch(
            f"{API_PREFIX}/bots/{NONEXISTENT_BOT_ID}/api_key", headers=auth_headers
        )

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_update_bot_api_key_without_auth_returns_401(
        self, client, invalid_auth_headers, created_bot
    ):
        response = client.patch(
            f"{API_PREFIX}/bots/{created_bot["bot_info"]["bot_id"]}/api_key",
...
class TestDeleteBotByID:
    def test_delete_bot_by_id_returns_204(self, client, auth_headers, sample_bot_data):
        ...
        response = client.delete(
            f"{API_PREFIX}/bots/{bot["bot_id"]}", headers=auth_headers
        )

        assert response.status_code == status.HTTP_204_NO_CONTENT

    def test_delete_bot_by_id_for_nonexistent_bot_returns_404(
        self, client, auth_headers
    ):
        response = client.delete(
            f"{API_PREFIX}/bots/{NONEXISTENT_BOT_ID}", headers=auth_headers
    ...
    def test_delete_bot_by_id_without_auth_returns_401(
        ...
        response = client.delete(
            f"{API_PREFIX}/bots/{bot["bot_id"]}", headers=invalid_auth_headers
        )
